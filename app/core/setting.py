...
class Settings(BaseSettings):
    ...
    app_env: str = Field("local", validation_alias=AliasChoices("APP_ENV", "ENVIRONMENT"))
    
    # Database
    database_url: str = Field(..., validation_alias="DATABASE_URL")
    database_pool_size: int = Field(10, ge=1, validation_alias="DATABASE_POOL_SIZE")
    database_max_overflow: int = Field(20, ge=0, validation_alias="DATABASE_MAX_OVERFLOW")
    database_pool_timeout_seconds: int = Field(30, ge=1, validation_alias="DATABASE_POOL_TIMEOUT_SECONDS")
    database_pool_recycle_seconds: int = Field(1800, ge=1, validation_alias="DATABASE_POOL_RECYCLE_SECONDS")
    ...
    request_id_max_length: int = Field(128, ge=16, le=512, validation_alias="REQUEST_ID_MAX_LENGTH")

    # LLM provider
    llm_api_key: str = Field("", validation_alias=AliasChoices("LLM_API_KEY", "GEMINI_API_KEY"))
    llm_model: str = Field("gemini-2.5-flash", validation_alias=AliasChoices("LLM_MODEL", "GEMINI_MODEL"))
