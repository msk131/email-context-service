...
| `FirmMembership` | Join table between user and firm. Stores firm-scoped roles such as `firm_admin` and `accountant`. |
...
- `firm_admin`: firm role on `firm_memberships`; can manage users/clients in that firm and view firm coverage reports.
- `accountant`: firm role on `firm_memberships`; can access clients and emails inside that firm. Accountant-specific profile data lives in `accountants`.
