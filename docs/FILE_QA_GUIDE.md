...
- `app/cache/lru.py` - Small TTL/LRU cache implementation.
- `app/cache/cache.py` - Summary cache wrapper functions. Keeps summary caching
...
- `app/models/__init__.py` - Imports model classes so Alembic/SQLAlchemy can see
  metadata.
- `app/models/users.py` - `User` and `FirmMembership` ORM models. Users
  authenticate, and memberships connect users to firms with roles.
- `app/models/accountants.py` - Accountant profile model for accountant-specific
  business data tied to a user and firm membership.
- `app/models/firms.py` - `Firm` ORM model.
- `app/models/clients.py` - `Client` ORM model with firm relationship and unique
  client email constraints.
- `app/models/summaries.py` - `Email`, `EmailSummary`, and `SummarizationLog`
  models. This is where encrypted summary text, token usage, email count, and
  refresh timestamp are stored.
- `app/models/tasks.py` - DB-backed background task model with status, payload,
...
- `app/repositories/emails.py` - Email insert/read queries.
- `app/repositories/summaries.py` - Summary/email/report/search queries, including
  date-range email reads and newly captured email counts.
...
- `app/services/emails.py` - Mock email send/receive workflows and client email
  read behavior.
- `app/services/summaries.py` - Compatibility facade that re-exports summary
  service functions from smaller modules.
- `app/services/summary_refresh.py` - Core summary generation workflow: date range
  validation, partial refresh skip, LLM call, encryption, token tracking, logs,
  cache invalidation, and preserving old summary on failure.
- `app/services/summary_cache.py` - Authorized cached summary reads.
- `app/services/summary_mapping.py` - Converts ORM summary records into API
  responses and decrypts summary text for authorized reads.
- `app/services/summary_reports.py` - Firm and global summary coverage reports.
- `app/services/summary_tasks.py` - Enqueues summary refresh work into the DB task
  queue after checking client authorization.
...
- `app/services/conversation.py` - Conversational question-answer flow over email
  context.
- `app/services/search.py` - Search helper utilities used by higher-level search
  services.
...
- `tests/cache/test_cache_cache.py` - Additional cache wrapper tests.
...
- "Where is summary encryption?" - `app/utils/helpers.py` encrypts/decrypts;
  `app/services/summary_refresh.py` writes encrypted text;
  `app/services/summary_mapping.py` decrypts for authorized responses.
...
- "Where is partial refresh?" - `app/services/summary_refresh.py`.
- "Where is caching?" - `app/cache/*` and `app/services/summary_cache.py`.
- "Where is tracking?" - `EmailSummary` and `SummarizationLog` in
  `app/models/summaries.py`; writes happen in `app/services/summary_refresh.py`.
- "Where are reports?" - `app/api/v1/summaries.py`,
  `app/services/summary_reports.py`, and `app/repositories/summaries.py`.
- "Where is background work?" - `app/api/v1/tasks.py`,
  `app/services/summary_tasks.py`, `app/services/tasks.py`,
  `app/repositories/tasks.py`, and `app/tasks/worker.py`.
