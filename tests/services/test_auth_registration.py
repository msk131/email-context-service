...
from app.services.auth import register_accountant
...
async def test_register_additional_user_requires_auth(monkeypatch):
    async def fake_get_accountant_by_email(session, email):
        return None
...
async def test_register_additional_user_with_firm_admin(monkeypatch):
    ...
    user = await register_accountant(
        session,
        email="new.user@example.org",
        password="Password123!",
        role=Role.accountant,
